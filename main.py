...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    weights = []
    for weight in arr:
        a_star = AStar(heuristic_function_type=heuristic_type, heuristic_weight=weight,
...
def toy_map_problem_experiments():
    ...
    res = uc1.solve_problem(toy_map_problem)
    print(res)
...
def basic_mda_problem_experiments():
    ...
    uc1 = UniformCost()
    res = uc1.solve_problem(small_mda_problem_with_distance_cost)
    print(res)
...
def mda_problem_with_astar_experiments():
    ...
    al = AStar(MDAMaxAirDistHeuristic)
    sol = al.solve_problem(moderate_mda_problem_with_distance_cost)
    print(sol)

    # Ex.25
    # TODO: create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    al1 = AStar(MDASumAirDistHeuristic)
    sol1 = al1.solve_problem(moderate_mda_problem_with_distance_cost)
    print(sol1)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    ase = AStarEpsilon(MDAMSTAirDistHeuristic, within_focal_priority_function=within_focal_h_sum_priority_function,
...
def mda_problem_anytime_astar_experiments():
    ...
    # exit()  # TODO: remove!
    any = AnytimeAStar(MDAMSTAirDistHeuristic, max_nr_states_to_expand_per_iteration=1000)
    sol = any.solve_problem(moderate_mda_problem_with_distance_cost)
    print(sol)
